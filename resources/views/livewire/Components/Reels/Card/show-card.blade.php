    ...
    <div class="bg-white w-full dark:bg-gray-900 shadow-md max-w-md">
		...
		<div class="flex items-center justify-between mb-4">
			<div class="flex items-center space-x-2">
				<img src="{{ asset($reel->user->division->logo_url) }}" alt="User Avatar" class="w-8 h-8 rounded-full">
					<p class="text-gray-900 dark:text-white font-semibold">{{ $reel->user->division->name  }}</p>
			</div>
            <div class="text-gray-500 flex space-x-4 cursor-pointer">
                <!-- Three-dot menu icon -->
                <a href="https://wa.me/?text={{ urlencode(route('reel.show', $reel->slug)) }}" target="_blank"
                    class=" text-gray-900 dark:text-white rounded-full p-1">
        ...
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center space-x-2">
                ...
                <div>
                    <p class="text-gray-800 dark:text-white font-semibold">{{ $reel->user->name }}</p>
                    <p class="text-gray-500 dark:text-white/50 text-sm">{{ $reel->created_at->diffForHumans() }}</p>
                </div>
            </div>
        ...
        <!-- Message -->
        <div class="mb-4">
            <p class="text-gray-800 dark:text-white ">{!! ($reel->content) !!}</p>
        </div>
		...
		<div class="flex px-4 items-center justify-between text-gray-500">
			<div class="flex items-center space-x-2">
                ...
                <button wire:click="like({{ $reel->id }})" class="flex items-center text-sm text-gray-500 hover:underline dark:text-gray-400 font-medium">
                    <svg class="mr-1.5 w-5 h-5 fill-current {{ $reel->likes->contains('user_id', Auth::id()) ? 'text-red-600' : '' }}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                        <path d="M12 21.35l-1.45-1.32C6.11 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-4.11 6.86-8.55 11.54L12 21.35z" />
                    </svg>
                    <span>{{ $reel->likes_count}} {{ $reel->likes->contains('user_id', Auth::id()) ? 'Suka' : 'Suka' }}</span>
                </button>
                @else
